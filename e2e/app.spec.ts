...
test('Canada scope offers both front-plate observations and changes the province chart', async ({ page }) => {
  await page.goto('/')
  await page.locator('.country-picker summary').click()
  await page.getByLabel('Canada', { exact: true }).check()
  await clueSearch(page).fill('Vehicle without a front plate')
  const without = page.locator('.text-clue').filter({ hasText: 'Vehicle without a front plate' })
  await expect(without).toHaveCount(1)
  await without.locator('.text-clue-pick').click()
  await expect(without.locator('.text-clue-pick')).toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.region-card')).toBeVisible()
  await expect(page.locator('.region-card [data-rank-id="loc:canada:region:ca-ab"]')).toBeVisible()
  await clueSearch(page).fill('Vehicle with a front plate')
  const withPlate = page.locator('.clue-card').filter({ hasText: 'Vehicle with a front plate' })
  await expect(withPlate).toHaveCount(1)
  await withPlate.locator('.clue-main').click()
  await expect(page.locator('.selection-chip')).toHaveCount(2)
  await expect(page.locator('.region-card .rank-row')).toHaveCount(6)
  await clueSearch(page).fill('MAXIMUM on a speed sign')
  await expect(page.locator('.clue-card, .text-clue')).toHaveCount(0)
  await page.getByRole('button', { name: 'All cited clues' }).click()
  await expect(page.locator('.clue-card').filter({ hasText: 'MAXIMUM on a speed sign' })).toHaveCount(1)
  await expect(page.getByText('these may leave the region chart unchanged.', { exact: false })).toBeVisible()
  await page.getByRole('button', { name: 'Region clues' }).click()
  await expect(page.locator('.clue-card, .text-clue')).toHaveCount(0)
})
