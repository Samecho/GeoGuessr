...
`scripts/curated-clues.tsv` holds the earlier reviewed phrase mappings. `scripts/focused-clues.tsv` adds paragraph-linked Canada and Africa observations; `scripts/focused-regions.json` and `scripts/focused-comparisons.tsv` hold their conditional regions and explicit contrasts. `scripts/focused-observation-decompositions.json` splits 55 source scene descriptions into independently selectable road, relief, surface, water and vegetation observations; the original contextual wording stays in `focused-clues.tsv`, and shared facts prevent double counting. Two photo-reviewed landscape scenes add small estimated joint terms. `scripts/focused-clue-aliases.json` joins reviewed synonyms to one clue ID; `scripts/source-fact-corrections.json` removes false substring matches and separates incompatible sign shapes; `scripts/verified-driving-side.json` records only explicit source statements before adding left/right observation estimates; `scripts/visual-specificity-review.json` marks 94 additional concrete metas for source-traceable rarity tiers, and `scripts/focused-evidence-profiles.json` records the Acadian flag review. These review files are source keyed and can be rerun without duplicates. `scripts/manual-fact-clues.json`, `scripts/estimate-context-review.json`, `scripts/interaction-review.json`, and `scripts/photo-review.json` record special fact interpretations, corrected source context, accepted combinations and visually checked card images. `npm run data:curate` generates `playable-*.json` and `source-photo-assets.json`. **Only those playable files drive the UI and scoring**; the larger raw extraction stays available for audit and later review. Unknown source mentions do not become negative evidence. Every active estimate points to a local source fact and is marked as estimated rather than measured.

Current checked output: 133 candidates including Antarctica; 475 selectable bilingual clues (304 with reviewed source images, 171 text-only), 1,322 active country/region estimates, 20 complete region schemes (Canada, 18 African source chapters and Brazil), six source-traceable interaction rules (including regional and parent marginals), and a separate chapter browser for all 5,860 images. The focused pass includes 294 reviewed source rows, expanded into 366 atomic source-observation rows, plus 95 comparative estimates, and 95 source-backed feature-specific evidence profiles. Hard-scope buttons can combine continents and source-candidate regional presets (East/Southeast Asia, Eastern/Western/Northern Europe, Latin America); these are practical filters, not evidence. The gallery follows only the user-selected geographic scope: a single country with a complete region scheme shows clues that distinguish its regions; wider scopes show source-backed clues in any included candidate. Selected observations persist when the scope changes. These 18 African chapters are the full African candidate scope requested from the supplied local documentation. The source images are web-sized copies totaling about 351 MB. The original full-resolution archive is never committed. See [coverage](docs/DATA_COVERAGE.md), [clue audit](docs/CLUE_AUDIT_2026-09-26.md), [scoring](docs/SCORING.md), [image sources](docs/ASSETS.md), and [open work](docs/GAPS.md).

To add a clue, verify the visible feature in its chapter, then add a bilingual label and source mapping to `scripts/curated-clues.tsv` or a paragraph ID prefix to `scripts/focused-clues.tsv`. When a source row describes several independently visible attributes, keep the contextual row and add a reviewed split in `scripts/focused-observation-decompositions.json`; assign its photo only to a component that the image clearly shows. Separate components from one paragraph share an evidence group. Only add a joint likelihood term when the source establishes extra information in the combination, and mark its likelihood ratio as an estimate. If no parser phrase captures an older clue, cite the relevant fact IDs in `manual-fact-clues.json`. For a focused photo card, visually check the paragraph image and record its zero-based image index in `focused-clues.tsv`; use `:left` only when a left-aligned thumbnail keeps tutorial map insets out of the card. The unmodified full image remains in Info. To add a region, edit `focused-regions.json` and map only source-supported observations to it. A complete scheme must be mutually exclusive and cover its claimed scope. Do not infer an absent feature from a chapter's silence.

## Personal clue library

The **My library / 我的题库** tab lets a user create bilingual visual clues, upload or paste their own images, and set separate relative likelihood multipliers for seeing or clearly not seeing a clue in a country or supported region. It uses IndexedDB for local persistence and can export or merge a versioned JSON backup containing embedded images. The official and personal libraries have independent observation selections and scoring; the geographic hard scope is shared. Personal data is never sent to the Pages host. See [personal library format and scoring](docs/CUSTOM_LIBRARY.md).
